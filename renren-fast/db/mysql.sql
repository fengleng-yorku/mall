...
CREATE TABLE `sys_menu` (
  ...
  `parent_id` bigint COMMENT 'parent menu id, 0 for top-level menu',
  `name` varchar(50) COMMENT 'menu name',
  `url` varchar(200) COMMENT 'menu URL',
  `perms` varchar(500) COMMENT 'permissions (multiple separated by comma, e.g.: user:list,user:create)',
...
CREATE TABLE `sys_user_role` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8mb4 COMMENT='user role relation';
...
CREATE TABLE `sys_role_menu` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8mb4 COMMENT='role menu relation';
...
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8mb4 COMMENT='system config';
...
CREATE TABLE `sys_log` (
  ...
  `params` varchar(5000) COMMENT 'request params',
  `time` bigint NOT NULL COMMENT 'execution time (ms)',
...
CREATE TABLE `schedule_job` (
  ...
  `status` tinyint(4) DEFAULT NULL COMMENT 'job status: 0-normal, 1-paused',
  `remark` varchar(255) DEFAULT NULL COMMENT 'remark',
  `create_time` datetime DEFAULT NULL COMMENT 'create time',
  PRIMARY KEY (`job_id`)
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8mb4 COMMENT='scheduled job';

-- scheduled job log
CREATE TABLE `schedule_job_log` (
  `log_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'job log id',
  `job_id` bigint(20) NOT NULL COMMENT 'job id',
  `bean_name` varchar(200) DEFAULT NULL COMMENT 'spring bean name',
  `params` varchar(2000) DEFAULT NULL COMMENT 'params',
  `status` tinyint(4) NOT NULL COMMENT 'job status: 0-success, 1-failed',
  `error` varchar(2000) DEFAULT NULL COMMENT 'error message',
  `times` int(11) NOT NULL COMMENT 'execution time (ms)',
  `create_time` datetime DEFAULT NULL COMMENT 'create time',
  PRIMARY KEY (`log_id`),
  KEY `job_id` (`job_id`)
) ENGINE=`InnoDB` DEFAULT CHARACTER SET utf8mb4 COMMENT='scheduled job log';
...
INSERT INTO `sys_menu`(`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES (6, 1, 'Scheduled Jobs', 'job/schedule', NULL, 1, 'job', 5);
...
INSERT INTO `sys_menu`(`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES (9, 6, 'Edit', NULL, 'sys:schedule:update', 2, NULL, 0);
...
INSERT INTO `sys_menu`(`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES (13, 6, 'Run Now', NULL, 'sys:schedule:run', 2, NULL, 0);
...
INSERT INTO `sys_menu`(`menu_id`, `parent_id`, `name`, `url`, `perms`, `type`, `icon`, `order_num`) VALUES (27, 1, 'Config Management', 'sys/config', 'sys:config:list,sys:config:info,sys:config:save,sys:config:update,sys:config:delete', 1, 'config', 6);
...
-- quartz built-in table structure
